"""
Code from https://github.com/HazyResearch/safari/blob/main/src/tasks/torchmetrics.py  
...
class BitsPerCharacter(Perplexity):
    ...
    def update(self, preds: Tensor, target: Tensor, loss: Optional[Tensor] = None) -> None:  # type: ignore
        ...
        count = target.numel()
        if loss is None:
            loss = self.loss_fn(preds, target).double() / math.log(2)  # math.log(2) is ln(2)
            # Do the above bc BPC is cross-entropy with log_2; log_2(n) = log_e(n) / log_e(2)
        self.total_log_probs += loss * count
        self.count += count
        
    def compute(self) -> Tensor:
        return self.total_log_probs / self.count
